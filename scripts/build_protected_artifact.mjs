...
const safeOnlineScript = onlineScript
  .replace("e.public_path.replace(/^\\/CONTENT\\/levels\\//,'')", "(()=>{const p=e.public_path.split('/').slice(3);if(['A','B','C','P'].includes(p[0]))p.shift();return p.join('/')})()")
  .replace("if(!r.ok)throw new Error(String(r.status));return r.json()", "if(!r.ok){const body=await r.text();throw new Error(String(r.status)+' '+body)}return r.json()")
  .replace("const catalog=await json('/api/catalog');renderTree(catalog.entries||[])", "const cacheKey='dec-online-catalog-v2';let catalog=null;try{catalog=JSON.parse(sessionStorage.getItem(cacheKey)||'null')}catch(_e){}if(catalog&&Array.isArray(catalog.entries))renderTree(catalog.entries);const fresh=await json('/api/catalog');if(!catalog||JSON.stringify(fresh)!==JSON.stringify(catalog)){sessionStorage.setItem(cacheKey,JSON.stringify(fresh));renderTree(fresh.entries||[])}")
  .replace("boot()})();", "setTimeout(boot,300)})();");
